...
export default class Wallet extends Service {
  ...
  async findPoolNFT(launcherId: string) {
    return this.command('find_pool_nft', {launcherId});
  }

  async recoverPoolNFT(launcherId: string) {
      return this.command('recover_pool_nft', {launcherId});
  }
}
