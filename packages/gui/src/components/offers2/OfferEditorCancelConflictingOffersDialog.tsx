import { ConfirmDialog, Flex } from '@ball-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'BALL' && assetStatus.type === 'BALL') {
      // assetId might be either 'BALL' or 'UNKNOWN'(maybe fee) but here both are treated as just a BALL spending
