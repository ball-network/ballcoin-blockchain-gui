...
import { AlertDialog } from '@ball/core';
import {
  Typography,
  Button,
  Box,
...
import { ball_to_mojo } from '../../../util/ball';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      ballper_input.value === '' ||
      Number(ballper_input.value) === 0 ||
      !Number(ballper_input.value) ||
      isNaN(Number(ballper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const ballper = ball_to_mojo(ballper_input.value);
    const ballper_value = Number.parseInt(Number(ballper));
    ...
    // var fee = ball_to_mojo(fee_input.value);
