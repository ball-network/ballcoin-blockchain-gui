import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@ball-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@ball-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@ball-network/api-react';
import { catToMojo, ballToMojo } from '@ball-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedBALL = useWatch({
    name: 'requested.ball',
  });

  const offeredBALL = useWatch({
    name: 'offered.ball',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedBALL ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('ball', ballToMojo(amount))),
    ...(requestedTokens ?? [])
