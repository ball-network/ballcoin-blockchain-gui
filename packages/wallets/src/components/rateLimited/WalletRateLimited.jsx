...
import { AlertDialog, Card, Flex, ballToMojo, mojoToBallLocaleString } from '@ball-network/core';
import {
  send_transaction,
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (ballcoin per interval):{' '}
                  {mojoToBallLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToBallLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = ballToMojo(amount_input.value);
    const fee = ballToMojo(fee_input.value);
