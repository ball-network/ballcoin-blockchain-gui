import { Button, Flex, useOpenDialog, ballToMojo } from '@ball-network/core';
...
export default function WalletConnectOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderData) {
      // use new fee value
      const feeBall = offerBuilderData.offered.fee?.[0]?.amount;
      if (feeBall) {
        const feeMojos = feeBall ? ballToMojo(feeBall).toFixed() : '0';
        onChange({
          ...values,
          fee: feeMojos,
        });
