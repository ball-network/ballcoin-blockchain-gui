...
import ballEnvironment from '../util/ballEnvironment';
import ballConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!ballEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    ballConfig.loadConfig(ballEnvironment.getBallVersion());
    ...
    const createWindow = () => {
      ...
      if (!ballConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !ballConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (ballConfig.manageDaemonLifetime()) {
        ballEnvironment.startBallDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Ball Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/ball-network/ballcoin-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/ball-network/ballcoin-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/ball-network/ballcoin-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/ball-network/ballcoin-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'BallHub' }),
            click: () => {
              openExternal(
                'https://www.ball-hub.com',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Ball Forks Calculator' }),
            click: () => {
              openExternal(
                'https://ballforkscalculator.com',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Ballforks Blockchain' }),
            click: () => {
              openExternal(
                'https://ballforksblockchain.com',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/ball-network/ballcoin-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/ball_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/BallProject');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Ball Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Ball' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Ball Blockchain' }),
            click: () => {
              openAbout();
            },
          },
