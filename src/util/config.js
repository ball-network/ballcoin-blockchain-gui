...
global.daemon_rpc_ws = `wss://${self_hostname}:38881`;
...
function loadConfig(version) {
  try {
    ...
    // 1) BALL_ROOT environment variable
    // 2) version passed in and determined by the `ball version` call

    // check if BALL_ROOT is set. it overrides everything else
    const config_root_dir =
      'BALL_ROOT' in process.env
        ? process.env.BALL_ROOT
        : path.join(os.homedir(), '.ball', version);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 38881); // jshint ignore:line
