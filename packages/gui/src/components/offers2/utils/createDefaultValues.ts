...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (BALL), indicates whether a token or CAT has a default entry
 ...
 * for creating an BALL offer, a CAT offer, or an NFT offer. BALL and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      ball: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
