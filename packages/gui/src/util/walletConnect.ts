...
const log = initDebug('ball-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `ball_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.ball;
    if (!requiredNamespace) {
      throw new Error('Missing required ball namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['ball:testnet', 'ball:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `ball:${instance}:${fingerprint}`);
