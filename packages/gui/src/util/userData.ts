...
export function getUserDataDir(): string {
  const ballRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(ballRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const ballRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${ballRootUserDataPath}`);
  app.setPath('userData', ballRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(ballRootUserDataPath, 'Local Storage', 'leveldb');
