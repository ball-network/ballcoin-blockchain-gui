...
import { mojo_to_ball } from '../../util/ball';
import {
  calculatePoolReward,
...
async function computeNewPlotId(block) {
  ...
  if (!pool_public_key || !plot_public_key) {
    return undefined;
  }
...
export default function Block() {
  ...
  const poolReward = mojo_to_ball(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_ball(
    calculateBaseFarmerReward(blockRecord.height),
  );
  const communityReward = mojo_to_ball(
    calculateCommunityReward(blockRecord.height)
  );
  const timelordReward = mojo_to_ball(
    calculateTimelordFee(blockRecord.height)
  );

  const ballFees = blockRecord.fees
    ? mojo_to_ball(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={'https://alltheblocks.net/ball/address/'+(currencyCode
            ? toBech32m(
