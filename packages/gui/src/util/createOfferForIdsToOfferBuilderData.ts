import { toBech32m, WalletType } from '@ball-network/api';
import { mojoToBall, mojoToCAT } from '@ball-network/core';
...
export default function createOfferForIdsToOfferBuilderData(
      ...
      if (asset) {
        switch (asset.walletType) {
          case WalletType.STANDARD_WALLET:
            section.ball.push({ amount: mojoToBall(numericValue.abs()).toFixed() });
            break;
          case WalletType.CAT:
  ...
  if (fee) {
    offerBuilderData.offered.fee = [{ amount: mojoToBall(fee).toFixed() }];
  }
