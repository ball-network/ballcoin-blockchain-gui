...
const log = debug('ball-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.ballcoin.top';
...
async function postToOfferBin(
  ...
  if (testnet) {
    return 'https://www.ballcoin.top/offers';
  }
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50029, // Pairing doesn't exist e.g. BALL/RandoCoin
...
async function postToSpacescan(
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'tball' : 'ball'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
  ...
  return `https://www.spacescan.io/${testnet ? 'tball' : 'ball'}/offer/${id}`;
}
...
const KeybaseTeamName = 'ball_offers';
