import { OfferSummaryRecord, OfferTradeRecord } from '@ball-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@ball-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['ball', 'tball'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.BALL: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const ballMakerFee = mojoToBall(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(ballMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.BALL
        ? ballToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
