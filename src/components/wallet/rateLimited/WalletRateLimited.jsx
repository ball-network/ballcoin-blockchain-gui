...
import { AlertDialog, Card, Flex } from '@ball/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const ballper_input = ip_parsed.limit;
    ...
    const ballper_value = Number.parseInt(Number(ballper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (ball per interval):{' '}
                  {mojo_to_ball_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = ball_to_mojo(amount_input.value);
    const fee = ball_to_mojo(fee_input.value);
